...
<aerolineasAAAFC:layout pageName="vuelos">
	...
	<form:form modelAttribute="vuelo" class="form-horizontal" id="add-vuelo-form">
		...
		<c:if test="${not vuelo['new']}"><input type="hidden" name= "aeropuertoOrigen" value= "${vuelo.aeropuertoOrigen.id}"/></c:if>
		<c:if test="${not vuelo['new']}"><input type="hidden" name= "aeropuertoDestino" value= "${vuelo.aeropuertoDestino.id}"/></c:if>
					...
					<div class="col-sm-10">
						<select name="aeropuertoOrigen" id="aeropuertoOrigen">
							<c:forEach items="${aeropuertos}" var="codigo">
								<option value="${codigo.id}"><c:out value="${codigo.codigoIATA}, ${codigo.nombre}"/></option>
							</c:forEach>
						</select>
					</div>
				</div>
